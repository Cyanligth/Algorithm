...
namespace Iterator
{
    ...
    public class LinkedList<T> : IEnumerable<T>
    {
        ...
        {
            private LinkedList<T> linkList;     // 연결리스트형 변수
            private LinkedListNode<T> head;     // 연결리스트노드형 변수
            private T current;      // 일반화 현재값 변수

            internal Enumerator(LinkedList<T> linkList)     // 초기화
            {
                this.linkList = linkList;
            ...
            public T Current { get { return current; } }        // 호출되면 현재값 반환하는 함수

            object IEnumerator.Current { get { return current; } }      
            // 오브젝트형 반복기의 현재값. 리스트랑 다르게 인덱스가 없기에 오류확인 if문을 안거친다. 호출되면 현재값 반환.

            public void Dispose() { }       // 리스트떄랑 이하동문

            public bool MoveNext()      // 무브넥스트 함수, 반복기의 위치를 옮겨주고 참/거짓을 반환
            {
                if (head != null)       // 만약 머리가 없지 않다면 == 있으면
                {
                    current = head.Value;       // 현재값은 머리의 값이 된다. -중요, 주소가 아니라 값이다.
                    head = head.next;       // 머리의 다음을 머리로 정한다. - 여기서 머리는 그냥 변수 이름이다. 연결리스트의 머리를 뜻하지 않는다.
                    return true;        // 참 반환
                }
                else        // 그 외의 경우 == 머리가 없으면
                {
                    current = default(T);       // 현재값은 기본값이 된다
                    return false;       // 거짓 반환
                }
            }
            ...
            {
                this.head = linkList.head;      // 머리변수가 가리키는 주소가 연결리스트의 제일 앞이 된다. 
                current = default(T);       // 현재값을 공백으로
            }
        }
        public int Average(LinkedList<int> linkList)
        {
            int sum = 0;
            int ave = 0;
            foreach (int i in linkList) { sum = sum + i; }
            ave = sum/linkList.Count;
            return ave;
        }
    }
}