...
namespace Project_TextRPG.Util
{
    internal class AStar
    {
		 ...
		 * 이동거리+남은거리가 적은 순서대로 탐색
		 * 위의 값이 같은 경우 이동거리가 길고 남은거리가 짧은 경우부터 탐색
		 * 남은거리는 장해물이 있어도 무시하고 계산
		 ******************************************************/
        ...
        public static bool PathFinding(in bool[,] tileMap, in Point start, in Point end, out List<Point> path)
        {
            ...
            PriorityQueue<ASNode, int> nextPointPQ = new PriorityQueue<ASNode, int>();  // 정점이랑 f저장
            ...
            while (nextPointPQ.Count > 0)
            {
                ...
                for (int i = 0; i < Direction.Length; i++)
                {
                    ...
                    // 4-2. 탐색한 정점 만들기
                    int g = nextNode.g + ((nextNode.point.x == x || nextNode.point.y == y) ? CostStraight : CostDiagonal);  // nextNode.g + 10;
                    ...
                    if (nodes[y, x] == null ||      // 탐색하지 않은 정점이거나
                        nodes[y, x].f > newNode.f)  // 가중치가 높은 정점인 경우 = 나로부터 가는게 더 빠른 경우
                    {
                        nodes[y, x] = newNode;
        ...
        private class ASNode
        {
            ...
            public Point? parent;   // 이 정점을 탐색한 정점     // ? == null값을 넣을 수 있다
